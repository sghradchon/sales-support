...
interface Organization {
  ...
  siblingLevelOrgOrder: number; // 兄弟間の並び順
  memo: string;
}
...
// ★ insertPosition: 組織ノードの上/下どちらにドロップするか
type InsertPosition = 'before' | 'after' | null;

const STORAGE_KEY_ORG = 'o1pro_organizations_1';
const STORAGE_KEY_CONTACT = 'o1pro_contacts_1';
...
const SalesHeatMapPage: React.FC = () => {
  ...
  // ----------------------------------------------------------------
  // 2) orgData/contactData 変化で再レイアウト
  // ----------------------------------------------------------------
  useEffect(() => {
  ...
  const handleAddOrganization = () => {
    ...
    const newOrg: Organization = {
      ...
      upperLevelOrgId: '',
      siblingLevelOrgOrder: 999,
      memo: '',
    };
    const updatedOrgs = [...orgData, newOrg];
    setOrgData(updatedOrgs);
    localStorage.setItem(STORAGE_KEY_ORG, JSON.stringify(updatedOrgs));
    setNewOrgName('');
  };
  ...
  const onSvgPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    // ★★★ 組織をドラッグ中なら、どの組織ノードの上/下にいるか計算し表示
    if (draggingState?.entityType === 'org' && positionedRoot) {
      ...
      if (overOrgId) {
        ...
        if (node) {
          ...
          setInsertPosition(diagramY < centerY ? 'before' : 'after');
        }
      } else {
  ...
  const onSvgPointerUp = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (dragCandidate) {
      if (dragCandidate.entityType === 'contact') {
        ...
        const org = orgData.find((x) => x.organizationId === dragCandidate.entityId);
        if (org) {
          setSelectedEntity({ type: 'org', orgId: org.organizationId });
          onOpen();
        }
      }
      setDragCandidate(null);
  ...
  const dropEntity = (screenX: number, screenY: number) => {
    ...
    if (entityType === 'contact') {
      ...
      if (!org) return;
      if (org.organizationId === foundOrgId) return; // 同じ

      const dropTarget = orgData.find((o) => o.organizationId === foundOrgId);
      if (!dropTarget) {
        // root化
      ...
      if (org.upperLevelOrgId === dropTarget.upperLevelOrgId) {
        ...
        if (!insertPosition) {
          // fallback
          const updatedOrg = { ...org, siblingLevelOrgOrder: dropTarget.siblingLevelOrgOrder + 0.1 };
          updateOrganization(updatedOrg);
        } else {
          // もし 'before' なら dropTarget より少し小さい
          // もし 'after' なら dropTarget より少し大きい
          const offset = (insertPosition === 'before') ? -0.1 : 0.1;
          const newOrder = dropTarget.siblingLevelOrgOrder + offset;
          const updatedOrg = { ...org, siblingLevelOrgOrder: newOrder };
          updateOrganization(updatedOrg);
        }
      } else {
        // 親が変わる
        const newParentId = dropTarget.organizationId;
  ...
  // ★ 追加: Node情報を探索 (dragOverOrgId -> PositionedNode取得用)
  function findPositionedNode(node: PositionedNode, orgId: string): PositionedNode | null {
    if (node.org.organizationId === orgId) {
      return node;
    }
    for (const child of node.children) {
      const found = findPositionedNode(child, orgId);
      if (found) return found;
    }
    return null;
  }
  ...
  const renderTree = (node: PositionedNode, parent?: PositionedNode) => {
    ...
    return (
      <React.Fragment key={node.org.organizationId}>
        ...
        {/* ★★ ドラッグ中のハイライト: 「ここにドロップすると入るよ」(線) */}
        {dragOverOrgId === node.org.organizationId && draggingState?.entityType === 'org' && (
          <InsertionIndicator node={node} position={insertPosition} />
        )}
    ...
    if (selectedEntity.type === 'org') {
      ...
      return (
        <>
          ...
          <ModalBody>
            ...
            <Box>名前: {org.organizationName}</Box>
            <Box>親: {org.upperLevelOrgId}</Box>
      ...
      return (
        <>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Box>{c.lastName} {c.firstName}</Box>
            <Box>{c.title}</Box>
            <Box>{c.contactDescription}</Box>
  ...
  const renderDragGhost = () => {
    ...
    if (entityType === 'contact') {
      ...
      return <div style={style}>{c.lastName} {c.firstName}</div>;
    } else {
  ...
  return (
    <Box p={4}>
      <Box as="h1" fontSize="xl" mb={4}>
        Sales Heat Map: Sibling Reorder + "Drop" Indicator
      </Box>
      ...
      <Box border="1px solid #ccc" w={`${svgWidth}px`} h={`${svgHeight}px`} position="relative">
        <svg
          ref={svgRef}
          width={svgWidth}
          height={svgHeight}
...
/** ★★★ 組織ノード上に「ドロップインジケータ」を表示するコンポーネント ★★★ */
const InsertionIndicator: React.FC<{ node: PositionedNode; position: InsertPosition }> = ({
  node,
  position,
}) => {
  // "before" → ノードの上辺、 "after" → ノードの下辺
  let lineY = node.y;
  if (position === 'after') {
    lineY = node.y + node.height;
  }
  return (
    <line
      x1={node.x}
...
function layoutOrgTree(root: OrgTreeNode): PositionedNode {
  ...
  function computePos(node: OrgTreeNode, startX: number, startY: number): PositionedNode {
    ...
    if (isLeaf) {
      ...
      const contactAreaHeight = cCount * CONTACT_BOX_HEIGHT
